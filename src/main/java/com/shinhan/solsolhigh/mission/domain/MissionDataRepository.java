...
public interface MissionDataRepository extends Repository<MissionData, Integer> {
    ...
    @Query("SELECT new com.shinhan.solsolhigh.mission.ui.dto.MissionView(m.child.nickname, m.child.name, m.id, m.description, m.isFinished, m.startAt, m.endAt, m.level) FROM MissionData m JOIN Child c ON m.child.id = c.id WHERE c.parent.id = :parentId AND m.isFinished = :isFinished ORDER BY m.id DESC")
    ...
    Slice<MissionView> findMissionDataByChild_IdAndIsFinished(Pageable pageable, @Param("isFinished") Boolean isFinished, @Param("childId") Integer childId);
