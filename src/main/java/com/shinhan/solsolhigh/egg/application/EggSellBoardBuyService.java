...
public class EggSellBoardBuyService {
    ...
    @Transactional
    public void buyEggSellBoard(EggSellBoardBuyRequest request, Integer sessionId) {
        ...
        EggCount byOwner = eggCountRepository.findByChild_Id(getBoard.getChild().getId()).orElseThrow(EggCountNotSufficientException::new);

        if(byOwner.getChild().equals(getBoard.getChild())) {
            throw new EggSellBoardCanNotSellSameUserException();
        }

        if(!getBoard.canBuy(request)) {
        ...
        EggTradeLog build = EggTradeLog.builder().buyer(bySessionEggCount.getChild()).seller(getBoard.getChild()).specialEgg(getBoard.getSpecialEgg()).eggStockCount(request.getEggCount()).eggPricePerOnce(getBoard.getEggPricePerOnce()).build();
